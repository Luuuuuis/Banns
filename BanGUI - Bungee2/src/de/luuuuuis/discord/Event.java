            ...
            if(textChannel.getIdLong() == Long.parseLong(Info.Discord.get("Channel-ID").toString())) {
            	...
            	if(msg.startsWith("/ban")) {
            		...
            		if(member.hasPermission(Permission.BAN_MEMBERS)) {
            			...
            			jda.getTextChannelById((String) Info.Discord.get("Channel-ID")).sendMessage(embed.build()).queue(m -> {
            				
            				m.addReaction(one).queue();
            				m.addReaction(two).queue();
            				m.addReaction(three).queue();
            				
            				exec.execute(new MessageRunnable(textChannel, m.getIdLong(), jda.getUserById(author.getIdLong()), toBan, author.getName(), member));
            			});
            		...
            		if(member.hasPermission(Permission.BAN_MEMBERS)) {
            		      ...
            		      if(banInfo != null) {
            		    	  if(banInfo.perm) {
                    			...
                    			jda.getTextChannelById((String) Info.Discord.get("Channel-ID")).sendMessage("More Informations under http://" + Info.ServerInfo.get("domainname") + ":1337/check?username=" + toCheck).queue();
...
static class MessageRunnable implements Runnable {
		...
		public MessageRunnable(TextChannel textChannel, long messageID, User user, String toBann, String banner, Member member) {
		...
		public void run() {			
			...
			for(int i = 0; i < 10; i++) {
				...
				for(MessageReaction reaction : message.getReactions()) {
					...
					if(name.startsWith("1") && reaction.getCount() > 1 && reaction.getUsers().complete().contains(user)) {
						reason = "Hacking";
						ban(member, toBann, reason, banner.replaceAll("[^\\p{ASCII}]", ""), 604800L, 0);
						exit=true;
					} else if(name.startsWith("2") && reaction.getCount() > 1 && reaction.getUsers().complete().contains(user)) {
						reason = "Hacking";
						ban(member, toBann, reason, banner.replaceAll("[^\\p{ASCII}]", ""), 2592000L, 0);
						exit=true;
					}
					if(name.startsWith("3") && reaction.getCount() > 1 && reaction.getUsers().complete().contains(user)) {
						reason = "Hacking";
			  ...
			  Banns.dcbot.onBann(toBan, reason, TimeManager.calc(time / 1000L));
			    ...
			    ProxyServer.getInstance().getPlayers().forEach(all -> {
		    		if(all.hasPermission(Banns.getBanperm())) {
		    				all.sendMessage(Banns.getPrefix() + "§e" + banner + " §7banned §c" + toBan + "§7 for §c§l" + reason + "§7.");
		    		}
			    });
