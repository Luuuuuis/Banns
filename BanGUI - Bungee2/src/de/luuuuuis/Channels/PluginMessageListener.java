...
public class PluginMessageListener implements Listener {
	...
	public static void bann(String nameName, String reason, String bannerName, Long time, String type) {
	      ...
	      ProxiedPlayer banner = ProxyServer.getInstance().getPlayer(bannerName), name = BungeeCord.getInstance().getPlayer(nameName);
	      ...
	      if(type.equals("BAN")) {
	    	  ...
	    	  if(banner.hasPermission(Banns.getBanperm())) {
	    		  if(name != null) {
		    		  if(!name.hasPermission(Banns.getBanperm())) {
		    			  ...
		    			  banner.sendMessage(new TextComponent(Banns.getPrefix() + "§4You are not allowed to bann team members."));
		    			  return;
		    		  }
	    		  }
	    		  ...
	    		  BanSQLHandler.createBan(uuid, reason, ip, bannerName, time, perma);
	    		  ...
	    		  Banns.dcbot.onBann(nameName, reason, TimeManager.calc(time / 1000L));
				    ...
				    ProxyServer.getInstance().getPlayers().forEach(all -> {
			    		if(all.hasPermission(Banns.getBanperm())) {
			    				all.sendMessage(Banns.getPrefix() + "§e" + banner.getDisplayName() + " §7banned §c" + nameName + "§7 for §c§l" + reason + "§7.");
			    		}
				    });
	    		  
	    	  } else {
	    		  banner.sendMessage(Banns.getPrefix() + "§4You don't have proxy permissions! Please report this to an administrator!");
	    	  }
	    	  ...
	    	  if(banner.hasPermission(Banns.getBanperm())) {
	    		  ...
	    		  banner.sendMessage(new TextComponent(Banns.getPrefix() + "§7You muted §c" + nameName + "§7 for §e" + reason));
	    		  ...
	    		  Info.LastMuteTime = TimeManager.calc(time / 1000L);
				    ...
				    ProxyServer.getInstance().getPlayers().forEach(all -> {
			    		if(all.hasPermission(Banns.getBanperm())) {
			    				all.sendMessage(Banns.getPrefix() + "§e" + banner.getDisplayName() + " §7muted §c" + nameName + "§7 for §c§l" + reason + "§7.");
			    		}
				    });
	    	  ...
	    	  if(BanSQLHandler.playerExists(uuid)) {
	    		  BanSQLHandler.unban(uuid);
	    		  banner.sendMessage(new TextComponent(Banns.getPrefix() + "§aYou unbaned §e" + nameName + " §asuccessfully."));
	    	  } else {
	    		  banner.sendMessage(new TextComponent(Banns.getPrefix() + "The player §e" + nameName + " §7isn't banned!"));
	    	  }
