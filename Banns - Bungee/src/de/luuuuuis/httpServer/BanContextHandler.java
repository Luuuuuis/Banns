		...
		try {
			...
			if (s != null && s.isValid(exchange)/* and session is valid */) {
				...
				String banListe = "<table id=\"bantable\" class=\"table tabel-hover table-light table-sm\"style=\"width:100%\">"
						...
						+ "<tbody>" + bList + "</tbody></table>";

				if (Banns.update) {

					result = result.replace("%servername%", (CharSequence) Info.ServerInfo.get("domainname"))
					...
					result = result.replace("%servername%", (CharSequence) Info.ServerInfo.get("domainname"))
							.replace("%banliste%", banListe).replace("%update%", "");

				}
			} else {
				result = readSite("login.html").replace("%servername%",
						(CharSequence) Info.ServerInfo.get("domainname"));
			}
	...
	private String getBanListe() throws SQLException {
		...
		try {
			...
			for (BanInfo banInfo : BanInfo.getInfos()) {
				...
				String content = "<tr><td>" + banInfo.uuid + "</td><td>"
						+ MojangUUIDResolve.getNameResult(banInfo.uuid).getValue() + "</td><td>" + Time + "</td><td>"
						+ replaceAllZahls(banInfo.reason) + "</td><td>" + banInfo.banner + "</td></tr>";
				bList += content;

			}

			for (MuteInfo muteInfo : MuteInfo.getInfos()) {
				...
				if (Time.equalsIgnoreCase("0 seconds")) {
					new Mute(muteInfo.uuid).unban();
					continue;
				}

				String content = "<tr><td>" + muteInfo.uuid + "</td><td>"
						+ MojangUUIDResolve.getNameResult(muteInfo.uuid).getValue() + "</td><td>" + Time + "</td><td>"
						+ replaceAllZahls(muteInfo.reason) + "</td><td>" + muteInfo.banner + "</td></tr>";
				bList += content;
			}
