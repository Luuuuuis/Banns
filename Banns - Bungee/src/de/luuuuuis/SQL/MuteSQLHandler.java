...
public class MuteSQLHandler {
	...
	public static void createPlayer(String uuid) {
		...
		if (!MuteSQLHandler.playerExists(uuid)) {
			MySQL.update("INSERT INTO MutedPlayers (UUID, BANNER, BANNED_TIME , BANNED_NEXT, REASON) VALUES ('" + uuid
					+ "', 'null', '" + banned_time + "','" + banned_next + "','" + reason + "');");
		}
	}
		...
		Thread th = new Thread(new Runnable() {
			...
			public void run() {
				if (MuteSQLHandler.playerExists(uuid)) {
					...
					MySQL.update("UPDATE MutedPlayers SET BANNED_TIME='" + currenttime + "', BANNED_NEXT='" + next
							+ "', REASON='" + reason + "', BANNER='" + Banner + "' WHERE UUID='" + uuid + "'");
